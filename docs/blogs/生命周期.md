...
对于`Activity`来说，当它进入到Resumed状态后，开发者可以轻易认为当前的`Activity`对于用户是「可见」的。但是对于处于`ViewPager`的离屏缓存区域的`Fragment`来说，虽然他们被加载出来并进入了`Resumed`状态，但是实际上用户是看不见这些`Fragment`。这就导致了生命周期与可见性产生了不同步的问题，毕竟`Resumed`的定义就是可以看见并可以操作的意思。
...
对于这个问题，谷歌为`Fragment`增加了一个`setUserVisibleHint(Boolean)`的方法来解决上述的问题。开发者可以手动通过调用这个方法来修改`Fragment`的可见度。需要注意的是，这个机制只是一个简单的「标记」，它并不能真正决定`Fragment`是否对于用户可见，笔者再次强调这只是一个「标记」。

于是`ViewPager`在页面跳转的时候，会主动去修改那些`Fragment`的`UserVisibleHint`，开发者再根据这个值来判断`Fragment`是否可见。
